      ...
      {carbs !== null ? (
        ...
        <p className="leading-7 [&:not(:first-child)]:mt-6">
          ...
          This means you should take in: <br />
            {carbs !== null && (
            <>
              {`- ${Math.round(carbs / (values.workoutDuration.hours + values.workoutDuration.minutes / 60))}g per hour`}
              <br />
              {`- ${Math.round(carbs / ((values.workoutDuration.hours * 60 + values.workoutDuration.minutes) / 20))}g every 20 minutes`}
              <br />
              {`- ${Math.round(carbs / ((values.workoutDuration.hours * 60 + values.workoutDuration.minutes) / 30))}g every 30 minutes`}
            </>
            )}
        </p>
        </>
