...
export function carbDosageToTime(formResponse: FormResponse): DosageResponse {
  ...
  const dosage: DosageResponse = {
    "20m": Math.round(totalCarbs /(durationInMinutes / 20 * 10)) / 10, // rounding to 2 decimal places
    "30m": Math.round(totalCarbs /(durationInMinutes / 30 * 10)) / 10,
    "60m": Math.round(totalCarbs /(durationInMinutes / 60 * 10)) / 10,
  };
